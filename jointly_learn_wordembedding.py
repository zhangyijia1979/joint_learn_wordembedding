...
def parse_args():
	...
	parser.add_argument('--input_corpus', nargs='?', default='./data/pubmed_corpus_sample.txt',
	...
	parser.add_argument('--input_dic', nargs='?', default='./data/mesh_dic_all.pkl.gz',
						help='Input mesh dic')
...
def main(args):
	#pubmed_file='C:/Users/zhangy51/word2vec/pubmed_corpus_new.txt'
	#dic_file='../graph/mesh_dic_all.pkl.gz'
	f_pkl = gzip.open(args.input_dic, 'r')
	mesh_dic=pkl.load(f_pkl)
	mesh2text = pkl.load(f_pkl)
	mesh2prob= pkl.load(f_pkl)
	...
	for instance in walks:
		...
		for node in instance:
			...
			if node in mesh_dic:
				...
				temp_list.append(mesh2text[mesh_dic[node]][random_index(mesh2prob[mesh_dic[node]])])
			elif node in supplement_dict:
